...
namespace AnimalManagement
{
    public partial class frmTimKiemHD : Form
    {
        ...
        private void btnTimKiem_Click(object sender, EventArgs e)
        {
            ...
            if (txtMaHDBan.Text != "")
                sql = sql + " AND bill_id Like N'%" + txtMaHDBan.Text + "%'";
            if (txtThang.Text != "")
                sql = sql + " AND MONTH(bill_date) =" + txtThang.Text;
            if (txtNam.Text != "")
                sql = sql + " AND YEAR(bill_date) =" + txtNam.Text;
            if (txtMaNhanVien.Text != "")
                sql = sql + " AND staff_id Like N'%" + txtMaNhanVien.Text + "%'";
            if (txtMaKhach.Text != "")
                sql = sql + " AND customer_id Like N'%" + txtMaKhach.Text + "%'";
            if (txtTongTien.Text != "")
                sql = sql + " AND total_price <=" + txtTongTien.Text;
        ...
        private void dgvTKHoaDon_DoubleClick(object sender, EventArgs e)
        {
            ...
            {
                mahd = dgvTKHoaDon.CurrentRow.Cells["bill_id"].Value.ToString();
